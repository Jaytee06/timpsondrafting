...
function renderCity(city) {
  ...
  const codeProfile = renderCodeProfile(city);
  ...
  const body = `<section class="hero city-hero"><div class="city-hero-copy"><p class="eyebrow">${escapeHtml(city.region)} service area</p><h1>Residential Drafting Services in ${escapeHtml(city.city)}, ${escapeHtml(city.state)}</h1><p>${escapeHtml(city.introduction)}</p><div class="actions"><a class="button primary" data-track="quote_form_start" href="${quoteUrl}">Request a Quote</a><a class="button secondary" href="tel:+14353195331">Call (435) 319-5331</a></div></div><img class="city-hero-image" src="/timpson-banner.jpg" width="640" height="426" alt="Residential drafting plans representing work in ${escapeHtml(city.city)}, ${escapeHtml(city.state)}"></section><section class="section"><h2>Local residential drafting support</h2><p>${escapeHtml(city.localOverview)}</p><div class="availability"><div><strong>Remote drafting</strong><span>${city.serviceAvailability.remoteDrafting ? 'Available for suitable projects' : 'Not currently offered'}</span></div><div><strong>On-site measurement</strong><span>${city.serviceAvailability.onsiteMeasurements ? 'May be available by scope' : 'Requires separate confirmation'}</span></div></div><p>${escapeHtml(city.serviceAvailability.onsiteSummary)}</p><p>To begin, provide the property location, intended work, approximate size, schedule, and any survey, photos, sketches, measurements, or existing plans.</p></section><section class="section"><h2>Featured residential drafting services</h2><div class="cards">${serviceCards}</div></section><section class="section"><h2>Common project types in ${escapeHtml(city.city)}</h2><ul>${city.commonProjectTypes.map((type) => `<li>${escapeHtml(type)}</li>`).join('')}</ul></section>${jurisdiction}${codeProfile}<section class="section"><h2>Nearby communities</h2><div class="nearby-list">${nearby}</div><p><a href="${regionUrl}">Explore residential drafting across ${escapeHtml(city.region)}</a>.</p></section><section class="section"><h2>Questions from ${escapeHtml(city.city)} project owners</h2>${city.faqs.map((faq) => `<details><summary>${escapeHtml(faq.question)}</summary><p>${escapeHtml(faq.answer)}</p></details>`).join('')}</section><section class="cta"><h2>Request a ${escapeHtml(city.city)} project quote</h2><p>Your city and state will be carried into the quote form. You can review or change them before submitting.</p><a class="button primary" data-track="quote_form_start" href="${quoteUrl}">Start your quote</a></section>`;
  return layout(city.seo, crumbs, body, jsonLd, city);
}

function renderCodeProfile(city) {
  const profile = city.codeProfile;
  if (!profile) return '';
  const statusText = profile.status === 'verified'
    ? 'Verified against the linked sources on the review date shown below'
    : profile.status === 'published-ordinance-needs-department-confirmation'
      ? 'Published ordinance—confirm current enforcement with the department'
      : 'Current status requires review before reliance';
  const authorityLinks = [
    `<a href="${escapeHtml(profile.authority.buildingDepartmentUrl)}" rel="noopener">Building department</a>`,
    profile.authority.permitPortalUrl ? `<a href="${escapeHtml(profile.authority.permitPortalUrl)}" rel="noopener">Permit portal</a>` : '',
    profile.authority.codeAdoptionUrl ? `<a href="${escapeHtml(profile.authority.codeAdoptionUrl)}" rel="noopener">Code-adoption source</a>` : '',
  ].filter(Boolean).join('');
  const adoptedCodes = profile.adoptedCodes.map((code) => `<tr><th scope="row">${escapeHtml(code.family)}</th><td>${escapeHtml(code.edition)}</td><td>${escapeHtml(code.appliesTo)} <a href="${escapeHtml(code.sourceUrl)}" rel="noopener">${escapeHtml(code.sourceLabel)}</a></td></tr>`).join('');
  const amendments = (profile.amendments || []).map((amendment) => `<li><strong>${escapeHtml(amendment.title)}:</strong> ${escapeHtml(amendment.summary)} <a href="${escapeHtml(amendment.sourceUrl)}" rel="noopener">Official source</a></li>`).join('');
  const criteria = (profile.localDesignCriteria || []).length ? `<h3>Published local design criteria</h3><div class="table-scroll" tabindex="0" aria-label="Published local design criteria table"><table class="code-table"><thead><tr><th scope="col">Criterion</th><th scope="col">Published value</th><th scope="col">Important context</th></tr></thead><tbody>${profile.localDesignCriteria.map((item) => `<tr><th scope="row">${escapeHtml(item.label)}</th><td>${escapeHtml(item.value)}</td><td>${escapeHtml(item.notes || '')} <a href="${escapeHtml(item.sourceUrl)}" rel="noopener">Source</a></td></tr>`).join('')}</tbody></table></div>` : '';
  const submissionItems = (profile.commonSubmissionItems || []).map((item) => `<li>${escapeHtml(item.item)} <span class="status-label">${escapeHtml(item.requiredStatus.replaceAll('-', ' '))}</span> <a href="${escapeHtml(item.sourceUrl)}" rel="noopener">Source</a></li>`).join('');
  const warnings = (profile.warnings || []).map((warning) => `<li>${escapeHtml(warning)}</li>`).join('');
  return `<section class="section code-profile"><p class="eyebrow">Verified planning reference</p><h2>Building Codes and Permit Context for ${escapeHtml(city.city)}</h2><p class="profile-status"><strong>Source status:</strong> ${escapeHtml(statusText)}</p><p><strong>Reviewing authority:</strong> ${escapeHtml(profile.authority.name)}${profile.authority.phone ? ` · ${escapeHtml(profile.authority.phone)}` : ''}</p><p>${escapeHtml(profile.summary)}</p><div class="source-links" aria-label="Official code and permit sources">${authorityLinks}</div><h3>Published adopted-code framework</h3><div class="table-scroll" tabindex="0" aria-label="Adopted code table"><table class="code-table"><thead><tr><th scope="col">Code</th><th scope="col">Edition</th><th scope="col">Typical relevance and source</th></tr></thead><tbody>${adoptedCodes}</tbody></table></div>${amendments ? `<h3>Amendments and local context</h3><ul>${amendments}</ul>` : ''}${criteria}<h3>Common early submission considerations</h3><ul>${submissionItems}</ul><aside class="code-disclaimer" aria-label="Code information disclaimer"><strong>Planning summary—not a code analysis</strong><p>This information supports early project planning and is not a complete code analysis. Adopted codes, amendments, zoning requirements, design criteria, and required professional services depend on the property and project scope. Confirm current requirements with the reviewing authority and applicable licensed professionals before relying on this information for design or construction.</p><ul>${warnings}</ul></aside><p class="reviewed">Sources last checked <time datetime="${profile.verifiedDate}">${formatDate(profile.verifiedDate)}</time>.</p></section>`;
}
